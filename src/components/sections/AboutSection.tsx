    ...
    <div className="space-y-4">
      ...
      <div>
        <label
          ...
          className="block text-xs font-semibold text-zinc-400 uppercase tracking-wider mb-2"
        >
          Tagline <span className="text-zinc-650 normal-case font-normal">(optional)</span>
        </label>
        <input
          id="about-tagline"
          type="text"
      ...
      <div>
        <label
          ...
          className="block text-xs font-semibold text-zinc-400 uppercase tracking-wider mb-2"
        >
          Description <span className="text-zinc-650 normal-case font-normal">(optional)</span>
        </label>
        <textarea
          ...
          className="w-full h-36 bg-surface-page border border-warm-border rounded-lg p-3 text-sm
                     ...
                     focus:border-accent focus:ring-1 focus:ring-accent/40 transition-colors"
          placeholder="A brief description of what your project does, why it exists, and who it's for..."
          value={description}
          onChange={(e) => update({ description: e.target.value })}
        />
        <p className="text-xs text-zinc-650 mt-1">{description.length} characters</p>
      </div>
    </div>
  );
};
